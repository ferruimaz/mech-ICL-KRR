...
from experiment_utils.support import (  # noqa: E402
    MODELS,
    Config,
    checkpoint_path,
    get_device,
...
def krr_posterior_risk(Ktt: torch.Tensor, Kt: torch.Tensor, A: torch.Tensor) -> float:
    posterior_cov = Ktt - Kt @ torch.linalg.solve(A, Kt.T)
    return float(torch.trace(0.5 * (posterior_cov + posterior_cov.T)).clamp(min=FLOOR).item())
...
def append_operator_record(
    ...
    row.update(operator_metrics(S, T, y, F_y, eval_probes, eval_fd, A, krr_risk))
    if extra:
        row.update(extra)
    records.append(row)
...
def rank_curve_records(
    ...
    for rank in range(max_rank + 1):
        ...
        metrics = operator_metrics(S, T, y, F_y, eval_probes, eval_fd, A, krr_risk)
        row: Dict[str, object] = {
            "episode": episode,
            "probe_kind": probe_kind,
            "basis": basis_name,
        ...
        out.append(
            {
                ...
                "TQ_rho_operator_to_T": tq.get("rho_operator_to_T", float("nan")),
                "actLR_rho_operator_to_T": act.get("rho_operator_to_T", float("nan")),
                "actLR_rho_KRR_cost": float(act.get("rho_operator_to_T", float("nan")))
                - float(tq.get("rho_operator_to_T", float("nan"))),
                "TQ_X_use": tq["X_use"],
...
def append_interpolation_records(
    ...
    row.update(operator_metrics(S, T, y, F_y, probes, fd, A, krr_risk))
    records.append(row)
...
def write_text_summary(
    ...
    if actlr_summary:
        ...
        for row in actlr_summary:
            lines.append(
                ...
                + f"TQ rho={row.get('TQ_rho_operator_to_T_mean', float('nan')):.4g}, "
                + f"actLR rho={row.get('actLR_rho_operator_to_T_mean', float('nan')):.4g}"
            )
    if interp_summary:
        lines.append("")
    ...
    for row in metric_summary:
        ...
        lines.append(
            ...
            + f"rho={row.get('rho_operator_to_T_mean', float('nan')):.4g}, "
...
def run() -> None:
    ...
    for episode in range(args.episodes):
        ...
        x_tgt_cpu = x_tgt[0].detach().cpu().double()
        Ktt = x_tgt_cpu @ x_tgt_cpu.T
        n_ctx = K.shape[0]
        I = torch.eye(n_ctx, dtype=torch.float64)
        A = K + args.sigma2 * I
        T = Kt @ torch.linalg.solve(A, I)
        krr_risk = krr_posterior_risk(Ktt, Kt, A)
