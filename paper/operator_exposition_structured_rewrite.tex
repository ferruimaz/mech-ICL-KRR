...
Let \(f_{\mathrm{tgt}}\) denote the latent target function values in the fixed episode. For the
matched GP/KRR model,
\[
Ty=\mathbb{E}[f_{\mathrm{tgt}}\mid y,G],
\]
and the KRR posterior prediction risk is
\[
R_{\mathrm{KRR}}(G)
:=
...
\]
For any linear approximation \(S\) to \(T\),
\[
...
\]
Thus the natural scale for deciding whether a reduced operator is good enough for prediction is not
zero operator error, but excess error relative to the irreducible KRR risk. Define
\[
\rho_G(S)
:=
\frac{
...
excess prediction risk is the tail
\[
\sum_{i>r}s_i^2.
\]
...
In the experiments below we write \(r_T:=r_{5\%}\), i.e. \(\alpha=0.05\). Thus \(r_T\) is not the
algebraic rank of \(T\). It is the number of ridge-relevant directions needed so that discarding all
remaining modes would add at most five percent of the KRR posterior risk. If
\(\dim\spanop(Q)<r_T\), then no rank-\(\dim\spanop(Q)\) operator, even an optimally aligned one, can
reach that risk tolerance. If \(\dim\spanop(Q)\ge r_T\), success is still not automatic: the span
must also align with the prediction-relevant ridge directions.
...
\section{Layerwise Sufficiency Construction}
...
\(\rho_G(T_Q)\approx10.4\) for the response rank.
...
\midrule
raw & 0.01194 & 0.00726 & 0.00279 & 0.16583\\
response & 0.01239 & 0.01169 & 0.01901 & 0.24505\\
\bottomrule
\end{tabular}
\end{table}
...
\section{Experiment 2: Layerwise Sufficiency and Emergence}
...
\midrule
\(d_x=3\)  & 3.0  & 0 & 46.8 & 3.0  & 0.00617\\
\(d_x=5\)  & 5.0  & 0 & 46.9 & 5.0  & 0.00918\\
\(d_x=8\)  & 8.0  & 0 & 47.0 & 8.0  & 0.01250\\
\(d_x=10\) & 10.0 & 0 & 47.0 & 10.0 & 0.01433\\
\(d_x=15\) & 14.9 & 1 & 47.0 & 14.9 & 0.01735\\
\bottomrule
\end{tabular}
...
The old causal seed extractor is now best read as a separate causal-use diagnostic. It selected
directions by individual ablation damage and, on RBF, produced a span with roughly the right
dimension but the wrong orientation: its prediction-risk excess is far above the five-percent
tolerance. The revised Experiment 2
shows that this was not a model failure. The KRR subspace is present; the old extractor was not the
right way to recover it.
...
expose a high-rank simultaneous target operator. The checkpoint README also reports that the best
fixed-validation score at the final step was
\[
\mathrm{MSE}/\mathrm{MSE}_{\mathrm{KRR}}=1.638.
\]
Thus this is not a converged exact-KRR solver. It is a partially successful checkpoint whose
remaining prediction error should have a mechanistic explanation.

The diagnostic deliberately uses a richer geometry:
\[
n_{\mathrm{ctx}}=47,\qquad n_{\mathrm{tgt}}=40.
\]
This removes the training target cap and asks for a much richer simultaneous target operator than
the checkpoint saw directly during training.

\begin{table}[htbp]
\centering
\caption{\(d_x=40\) target-rich diagnostics for the older causal seed extractor. The seed extractor
is not the definition of task rank, but it remains useful for diagnosing alignment and model use.
The column \(\rho_G(T_Q)\) is computed on the prediction-risk scale.}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{0.9}
\begin{tabular}{llrrrrr}
\toprule
Run & Statistic & \(\dim Q\) & \(r_T\) & \(\rho_G(T_Q)\) & \(E(F,T)\) & MSE/KRR\\
\midrule
Seed extractor & all 16 episodes & 29.25 & 19.81 & 0.504 & 98.677 & 2428.685\\
Seed extractor & non-catastrophic 14 & 26.71 & 21.86 & 0.576 & 0.123 & 1.178\\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1}
\end{table}

The seed-extractor rows give the harder reading. Under the older relative-RMS interpretation it was
tempting to say that the seed span was simply too low-dimensional. The prediction-risk rank changes
that conclusion. On the non-catastrophic episodes the accepted seed dimension is about \(26.7\),
while \(r_T\) is about \(21.9\). Thus the span is not short relative to the prediction-risk rank.
Nevertheless
\[
\rho_G(T_{Q_{\mathrm{seed}}})\approx0.576,
\]
which is above the five-percent risk tolerance. The failure is therefore alignment, not merely
dimension: the seed extractor finds many directions, but not the right KRR-oriented subspace.

The revised layerwise sufficiency diagnostic gives an even cleaner separation. We reran the new
Experiment 2 on the same checkpoint using the native \(d_x=40\) spectrum-capped sampler, but with
the target-rich diagnostic geometry \(n_{\mathrm{ctx}}=47,n_{\mathrm{tgt}}=40\). In the
all-episode average the model-response error is dominated by two catastrophic episodes
\((E(F,T)=662.70\) and \(814.45)\), while the median is \(0.131\). In the remaining
non-catastrophic episodes, the response-only span becomes KRR-sufficient by layer 2:

\begin{table}[htbp]
\centering
\caption{Revised Experiment 2 on the \(d_x=40\) checkpoint, response basis, excluding the two
catastrophic episodes. Here \(\rho(T_Q)\) is the excess prediction risk of the reduced KRR operator
divided by the KRR posterior risk.}
\scriptsize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.9}
\begin{tabular}{rrrrr}
\toprule
Layer & cand. dim. & rank to 5\% & \(\rho(T_{Q_{\ell,r_T}})\) & best excess\\
\midrule
0 & 16.0 & --   & 1.330 & 1.330\\
1 & 45.9 & 21.9 & 0.056 & 0.014\\
2 & 46.9 & 21.9 & 0.043 & 0.001\\
3 & 47.0 & 21.9 & 0.042 & 0.000\\
8 & 47.0 & 21.9 & 0.042 & 0.000\\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1}
\end{table}

This table answers the availability question: the prediction-risk-rank KRR source subspace is
present by layer 2 on ordinary \(d_x=40\) episodes. It does not answer the use question. On the same
non-catastrophic subset,
\[
E(F,T)\approx0.127,
\qquad
\frac{\mathbb{E}\|(F-T)y\|^2}{R_{\mathrm{KRR}}}\approx0.158.
\]
So \(T_Q\) is close to \(T\), but the transformer's own local response is still noticeably imperfect.
The natural decomposition is
\[
F-T=(F-T_Q)+(T_Q-T).
\]
The new Experiment 2 makes the second term small. The remaining error is mostly in \(F-T_Q\): the
model has access to the right source subspace, but it does not reliably apply the exact
spectrum-dependent and target-dependent KRR readout from that subspace.

The catastrophic episodes sharpen this point. In the two worst episodes the seed dimension reaches
the full context cap, \(47\), so exact KRR is available through the extracted span. Yet the
transformer response itself is catastrophic, with \(E(F,T)\) of order \(10^3\) and
model/KRR MSE ratios \(3304.6\) and \(35537.9\). These episodes cannot be explained by a missing
KRR subspace. They show that the model map can fail even when the full reduced operator is
available. Across the remaining 14 episodes the model is much more moderate,
\[
E(F,T)\approx0.123,\qquad \text{MSE/KRR}\approx1.178,
\]
but the seed-induced operator still misses nontrivial KRR response.
...
pressure to build a stable reusable target-rich label-response operator. When the diagnostic
removes the target cap, it asks for a richer simultaneous computation than the checkpoint was
directly trained to expose. The \(d_x=40\) result should therefore be read as a regime-mismatch
stress test: the model contains substantial KRR-sufficient final-state and layerwise response
structure, but the selected causal seed span is misoriented relative to KRR on ordinary episodes,
the learned use of the available subspace is imperfect, and the model has rare catastrophic
failures in this richer regime. This agrees with the README-level MSE ratio. A checkpoint with
\(\mathrm{MSE}/\mathrm{MSE}_{\mathrm{KRR}}=1.638\) should not be expected to have
\(E(F,T)\) near zero; the operator diagnostics explain why the prediction error remains above KRR
even though the relevant KRR subspace is visible.

This motivates the sequence-length prescription above. If the goal is to train and certify a true
prediction-relevant KRR mechanism in a given regime, choose \(n_{\mathrm{ctx}}\) and \(n_{\mathrm{tgt}}\) so that
\[
\min\{n_{\mathrm{ctx}},n_{\mathrm{tgt}},r_*(n_{\mathrm{ctx}};0.05)\}
=r_*(n_{\mathrm{ctx}};0.05)
\]
...
\begin{table}[htbp]
\centering
\caption{RBF \(128\times128\) diagnostics. The layerwise response basis is the revised Experiment 2
basis. The seed row is the older individual-ablation extractor, included only to show why the older
causal basis should not be used as the primary emergence definition.}
\scriptsize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.9}
\begin{tabular}{lrrl}
...
Old causal seed & dim. \(22.9\) & \(\rho(T_Q)\approx0.889\) & wrong orientation\\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1}
\end{table}

The table gives a cleaner conclusion than the older seed-based analysis. The model itself is close
to KRR, and the final response span contains a rank-\(25\)-scale subspace whose induced reduced KRR
...
oriented by KRR relevance rather than by individual ablation damage.

We also reran the revised layerwise diagnostic on the older fixed-\(\ell=3\) RBF checkpoint
\path{model_rbf_fixed_l3.pt}, at \(n_{\mathrm{ctx}}=47\). This checkpoint was originally
...
The old causal seed extractor failed for a principled reason. It selected directions by
one-at-a-time intervention damage, which is basis-dependent and can miss distributed subspaces. On
the RBF checkpoint it produced roughly the right number of directions but the wrong orientation,
with
\[
\rho_G(T_{Q_{\mathrm{seed}}})\approx0.89.
\]
The revised Experiment 2 therefore changes the role of the causal basis. It is no longer the
primary definition of the KRR subspace. The primary object is the layerwise response span, ordered
by KRR-targeted energy and evaluated by prediction-risk excess. Under that definition:
\[
\text{RBF behavior and final-state availability succeed,}
\]
while
\[
\text{the older causal seed extraction remains only partially aligned with the KRR operator.}
\]
This distinction is the main lesson of the nonlinear checkpoint. The failure was not that the
directions were absent. It was that the previous causal-basis construction was not the right
subspace recovery method for a distributed nonlinear operator.
