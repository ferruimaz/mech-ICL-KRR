...
becomes available across layers, and whether removing KRR-important directions damages the model's
response.
...
on task and isotropic probes. Right: prediction-risk excess \(\rho_G(S)\), with the dotted line
marking the five-percent tolerance.
...
certificate; it is a fair matched-probe stress test asking whether the response subspace available
under that probe law still supports the KRR-constrained operator. Figure~\ref{fig:exp1-probe-interpolation}
separates the two relevant quantities: local response agreement on the mixed probes and
prediction-risk sufficiency of the induced KRR-constrained operator.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.90\linewidth]{../experiments/experiment_1_operator_certificate/results/probe_interpolation.png}
...
Left: finite-difference response errors on the mixed probe law. Right: task prediction-risk excess
\(\rho_G(T_{Q_\lambda})\), with the dotted line marking the five-percent tolerance. As
\(\lambda\) approaches \(1\), even the matched response basis no longer supports a clean
KRR-constrained explanation.}
...
Experiment 3 uses the same trained model as Experiment 1:
\path{linear_baseline_dx5_L8.pt} (originally \path{model_L8.pt}). The diagnostic is run on
flat-spectrum linear episodes with \(d_x=5\), \(n_{\mathrm{ctx}}=47\),
\(n_{\mathrm{tgt}}=16\), \(\sigma^2=0.1\), finite-difference scale \(\varepsilon=10^{-3}\), 32
episodes, and 32 held-out causal probes per episode. The complement-ablation run uses the same
...
subsets. Residual state \(s=0\) is the embedding output, \(s=i\) is the state after layer \(i\), and
\(s=8\) is the final residual state before the output head.

The Experiment 3 intervention metrics are normalized to be comparable across episodes. Prediction
drift is \(\|F_{\mathrm{surg}}(y)-F_{\mathrm{base}}(y)\|/\|F_{\mathrm{base}}(y)\|\). KRR-error
inflation is the post-surgery pointwise KRR error divided by the pre-surgery pointwise KRR error.
Response damage is
\[
\left(
...
\right)^{1/2},
\]
so it measures how much the intervention changes the local finite-difference response, on the same
task-label scale used for \(E_{\mathrm{task}}(F,T)\).
...
a prediction-risk-rank subspace whose induced reduced operator is prediction-risk sufficient; in the
...
becomes available; the causal intervention then asks whether the model actually uses the selected
directions.
\end{quote}
